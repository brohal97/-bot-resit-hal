...
// ✅ INDEX.JS PENUH: Jumlah + Tarikh + Bersih + Tanpa Crash
...
function extractTarikhList(text) {
  const patterns = [
    /\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/g,
    /\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}/g,
    /\d{1,2}\s+(jan|feb|mac|apr|may|jun|jul|aug|sep|oct|nov|dec)\s+\d{4}/gi,
    /(jan|feb|mac|apr|may|jun|jul|aug|sep|oct|nov|dec)[\s\-]+\d{1,2},?\s+\d{4}/gi
  ];
  let result = [];
  patterns.forEach(p => {
